...
\begin{hataanalizi}
Üç farklı aralık türünü karıştırmayınız:
\begin{itemize}
...
$X_1,\ldots,X_{20}$ gözlemi, $\bar{x}=12.4$, $s=3.1$. (a) Nonparametrik bootstrap ile medyan için \%95 GA'nın mantığını açıklayınız. (b) $B=1000$ bootstrap örnekleminde medyan standart hatası $\hat\sigma^*=0.82$ bulundu. Normal yaklaşım ile GA'yı kurunuz. (c) Bootstrap'ın klasik $t$-aralığına göre avantajı nedir?
\end{alistirma}
\alcozum{%
...
$X_1,\ldots,X_n\iid\Binom(1,p)$, $n=30$, $\sum x_i=18$. Önsel $p\sim\Beta(2,2)$. (a) Sonsal dağılımı bulunuz. (b) \%95 eşit-kuyruklu güvenilir aralığı Beta kantil tablosuyla kurunuz. (c) Bu aralığı klasik Wald aralığıyla karşılaştırınız.
\end{alistirma}
\alcozum{%
