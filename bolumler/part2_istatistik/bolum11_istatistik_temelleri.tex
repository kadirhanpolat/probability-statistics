...
\begin{kesif}
$X_1,\ldots,X_{100} \iid \Normal(\mu, 1)$ gözlemlendi.
$\bar{X}_{100}$'ü not ettikten sonra ham veriyi silsek, $\mu$ tahmini için
bir şey kaybeder miyiz? Peki ya yalnızca $X_1$'i tutup geri kalan
$99$ gözlemi silseydik?
\kesifcevap{$\bar{X}_{100}$'ü bildikten sonra ham veriyi silmek hiçbir bilgi
  kaybettirmez: $\mu$'ya dair her şey $\bar{X}$'te özetlenmiştir.
  Yalnızca $X_1$'i tutmak ise $99$ gözlemin bilgisini yok eder.
  Aradaki fark: $\bar{X}$ yeterli istatistik, $X_1$ tek başına değil.}
\end{kesif}

\begin{tanimgerekce}
Sezgi oturduğunda şu soru kalır: $T$'nin veriyi ``ne ölçüde özetlediğini''
nasıl matematiksel olarak sınarız? Cevap, $T = t$ koşuluyla $\mathbf{X}$'in
koşullu dağılımının $\theta$'ya bağlı olup olmadığıdır. Bağımlıysa ham veride
hâlâ $\theta$ bilgisi vardır; değilse $T$ tüm bilgiyi taşıyor demektir.
\end{tanimgerekce}
...
\begin{ozyansima}
\begin{itemize}
  \item "Parametrik model" ve "parametrik olmayan model" arasındaki farkı bir örnekle açıklayınız.
  \item Yeterli istatistiğin "veri özetleme" yorumunu ve pratikte neden önemli olduğunu anlatınız.
  \item $(\bar{X}_n, S^2)$'nin $\sigma^2$ bilinen $\Normal(\mu,\sigma^2)$'de yeterli istatistik olup
        olmadığını tartışınız. Tek bilinmeyen $\mu$ ise ne değişir?
...
\begin{kesif}
Bir madeni para $n = 10$ kez atıldı ve $7$ tura geldi. Frekansçı yaklaşımda
$\hat{p} = 0.7$ tahmini elde ederiz. ``Bu para adil mi?'' sorusuna
$P(\text{adil para} \mid \text{veri})$ diyebilir miyiz?
Bu ifade frekansçı çerçevede neden anlamsızdır; Bayesçi çerçevede ne anlam kazanır?
\kesifcevap{Frekansçı: $\theta\in\Theta$ sabit ama bilinmeyen; $P(\theta=\text{adil})$
  tanımsızdır çünkü $\theta$ rasgele değil. Güven aralığı parametreyi değil,
  prosedürü kapsıyor.
  Bayesçi: $\theta \sim \pi(\theta)$ önsel; $7$ tura sonrası
  $\pi(\theta \mid \mathbf{x}) \propto L(\theta;\mathbf{x})\pi(\theta)$ sonsal.
  ``Adil para'' olasılığı doğrudan $P(\theta=0.5 \mid \mathbf{x})$ ile hesaplanır;
  bu güven aralığından kavramsal olarak farklıdır.}
\end{kesif}
...
(a) $\bar{X}_5$'in dağılımını yazınız.
(b) $P(\bar{X}_5 > 4)$'ü hesaplayınız.
(c) $(n-1)S^2/\sigma^2$'nin dağılımını yazınız ve $E[S^2]$'yi bulunuz.
\end{alistirma}
\alcozum{%
...
(a) Ortak PMF'yi yazınız. (b) $T = \sum X_i$'nin yeterli istatistik olduğunu F-N teoremi ile gösteriniz.
(c) $T \sim \Pois(n\lambda)$ olduğunu belirtiniz.
\end{alistirma}
\alcozum{%
...
(a) $E[\tilde{S}^2]$ ve $\mathrm{Var}(\tilde{S}^2)$'yi bulunuz.
(b) $\mathrm{OKH}(\tilde{S}^2, \sigma^2)$'yi hesaplayınız.
(c) $\mathrm{OKH}(\tilde{S}^2,\sigma^2)$ ile $\mathrm{OKH}(S^2,\sigma^2)$'yi karşılaştırınız.
\end{alistirma}
\alcozum{%
...
(a) $T = X_{(n)} = \max X_i$'nin yeterli olduğunu F-N ile gösteriniz.
(b) $E_\theta[g(T)] = 0$ tüm $\theta > 0$'da koşulundan $g \equiv 0$ sonucunu türetiniz.
(c) $T$ tamamlayıcı mı?
\end{alistirma}
\alcozum{%
...
\begin{alistirma}[Al.11.4b]{B}{GBHK --- Normal Ailede}
$X_1,\ldots,X_n$ i.i.d.\ $\Normal(\mu,\sigma^2)$ ($\sigma^2$ bilinen).
(a) $T=\bar{X}$ için Cramér-Rao alt sınırını hesaplayınız.
(b) $\bar{X}$'in UMVUE olduğunu gösteriniz.
(c) $\sigma^2$ de bilinmiyorsa ne değişir? Yeterli ve tamamlayıcı istatistik nedir?
\end{alistirma}
\alcozum{%
(a) $\log f(x|\mu) = -\frac{(x-\mu)^2}{2\sigma^2} + C$; $\frac{\partial\log f}{\partial\mu}=\frac{x-\mu}{\sigma^2}$; Fisher bilgisi $I(\mu) = \frac{1}{\sigma^2}$. $n$ gözlem için $I_n(\mu)=n/\sigma^2$. GBHK: $\Var(\hat\mu)\geq\sigma^2/n$. $\Var(\bar{X})=\sigma^2/n$ alt sınıra eşit.

(b) $\bar{X}$ yansız. $T=\sum X_i$ yeterli ve tamamlayıcı (üstel aile). Lehmann-Scheffé: $T$'nin yansız fonksiyonu UMVUE'dir. $\bar{X}=T/n$ $\mu$'nün yansız tahmincisi, $T$'nin fonksiyonu $\Rightarrow$ UMVUE.

(c) $\sigma^2$ bilinmiyorsa, $(T_1,T_2)=(\sum X_i, \sum X_i^2)$ iki boyutlu yeterli ve tamamlayıcı istatistik (iki parametreli üstel aile). UMVUE çifti: $\bar{X}$ için $\mu$, $S^2$ için $\sigma^2$.}

\begin{alistirma}[Al.11.4c]{C}{Eksik Yeterlilik --- Cauchy Ailesi}
$X_1,\ldots,X_n$ i.i.d.\ $\mathrm{Cau}(\theta,1)$; $f(x|\theta)=\dfrac{1}{\pi[1+(x-\theta)^2]}$.
(a) $\bar{X}$'in yeterli istatistik olduğunu Fisher-Neyman ile kontrol etmeyi deneyin. Ne gözlemliyorsunuz?
(b) $X_{(1)}<\cdots<X_{(n)}$ sıra istatistikleri birlikte yeterli mi? Bu ne anlama gelir?
(c) Neden Cauchy ailesinde GBHK genellikle uygulanamaz?
\end{alistirma}
\alcozum{%
(a) Ortak yoğunluk: $f(\mathbf{x}|\theta)=\pi^{-n}\prod[1+(x_i-\theta)^2]^{-1}$. Bu çarpım $\theta$'dan kolay ayrışmaz; $g(T(\mathbf{x}),\theta)\cdot h(\mathbf{x})$ biçimine getirilemiyor çünkü $\theta$ her faktörde. $\bar X$ yeterli \emph{değildir}.

(b) $(X_{(1)},\ldots,X_{(n)})$ sıra istatistikleri her zaman \emph{tamamlayıcı} yeterli istatistik sağlar (Halmos-Savage). Bu, veriyi sıralama dışında bilgisiz perütmasyonla sunmaya eşdeğer. Fakat boyut azaltma sağlamaz; boyut yine $n$.

(c) GBHK için Fisher bilgisi $I(\theta)=\int\left(\frac{\partial\log f}{\partial\theta}\right)^2 f\,dx$'nin varlığı gerekir. Cauchy için $\partial\log f/\partial\theta = 2(x-\theta)/[1+(x-\theta)^2]$; $I(\theta)=1/2<\infty$. Teknik olarak GBHK var, fakat maksimum olabilirlik tahmincisi $\hat\theta_{\mathrm{MLE}}$ \emph{açık formda yok} (sayısal çözüm gerekir) ve $\bar X$'in Cauchy dağılımlı olup varyansı sonsuz olduğundan GBHK anlamsız.}
...
$T = \sqrt{16}(\bar{X}_{16}-\mu)/S$'nin dağılımını yazınız.
$P(T > 2.131)$'i $t$ tablosundan söyleyiniz.
\end{alistirma}
\alcozum{%
$T \sim t_{15}$.
...
(a) Ortak PDF'yi üstel aile formunda yazınız. (b) Yeterli istatistiği bulunuz.
\end{alistirma}
\alcozum{%
...
$(Y_1, Y_2, Y_3) = (X_1+X_2+X_3, X_1-X_2, X_1+X_2-2X_3)/\sqrt{6}$ dönüşümünü yapınız.
(a) $Y_i$'lerin $\Normal(?, ?)$ dağılımını bulunuz.
(b) $Y_1$ ile $(Y_2, Y_3)$'ün bağımsız olduğunu gösteriniz.
(c) $\bar{X}$ ve $S^2$'nin bağımsızlığını bu sonuçtan çıkarınız.
\end{alistirma}
\alcozum{%
...
$f(\mathbf{x}|\theta)/f(\mathbf{y}|\theta)$ oranını yazınız.
