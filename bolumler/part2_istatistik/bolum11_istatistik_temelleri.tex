...
\kesifcevap{Fisher-Neyman çarpanlanma teoremi: $T$ yeterli $\Leftrightarrow$
  $f(\mathbf{x};\theta)=g(T(\mathbf{x});\theta)\cdot h(\mathbf{x})$.
  Yeterli istatistik, $\theta$'ya bağlı tüm bilgiyi içinde taşır;
  kalan veri $\theta$ için ek bilgi vermez. Normal'de $(\bar X, S^2)$ yeterlidir.}
\end{kesif}
...
\kesifcevap{Frekansçı: $\theta\in\Theta$ sabit; olasılık büyük örneklem frekansıdır.
  Bayesçi: $\theta\sim\pi(\theta)$; veriyi görerek $\pi(\theta|\mathbf{x})\propto L(\theta;\mathbf{x})\pi(\theta)$.
  Temel fark: parametrenin ontolojik statüsü --- sabit mi, rasgele mi?
  İkisi farklı hedefler seçer; güven aralığı $\neq$ credible interval.}
\end{kesif}
...
(a) $T=\bar{X}$ için Cramér-Rao alt sınırını hesaplayın.
(b) $\bar{X}$'in UMVUE olduğunu gösterin.
