...
\kesifcevap{Önsel $\pi(\theta)$ + olabilirlik $L(\theta;\mathbf{x})$ $\Rightarrow$ sonsal
  $\pi(\theta|\mathbf{x})\propto L(\theta;\mathbf{x})\pi(\theta)$.
  Bayes tahmini: kare kayıpta sonsal ortalama, mutlak kayıpta sonsal medyan, MAP ise sonsal mod.
  Konjugat önsel seçimi ($\pi$ ve sonsal aynı aile) hesabı kapalı formda verir.}
\end{kesif}
