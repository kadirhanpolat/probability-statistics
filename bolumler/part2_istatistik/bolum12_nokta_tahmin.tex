...
\kesifcevap{$\overline{X}_n$ yansız ($E[\overline{X}_n]=\mu$), tutarlı ($\overline{X}_n\xrightarrow{P}\mu$)
  ve Normal ailede UMVUE'dur. Alternatifler: medyan (sağlam, Normal'de $\%86$ etkin),
  kırpılmış ortalama (sağlamlık-etkinlik dengesi). MLE her zaman $\overline{X}_n$'i seçer.}
\end{kesif}
...
\kesifcevap{Cramér-Rao alt sınırı: $\mathrm{Var}(\hat\theta)\geq 1/(nI(\theta))$.
  Fisher bilgisi $I(\theta)=E[(\partial\log f/\partial\theta)^2]$.
  Yansız $\hat\theta$ bu sınıra ulaşırsa etkin (efficient) denir.
  Normal'de $\overline{X}_n$ $\mu$ için, $S^2$ $\sigma^2$ için sınıra ulaşır.}
\end{kesif}
...
\begin{kesif}
$X_1,\ldots,X_{10} \iid \mathrm{Ber}(p)$, $\sum x_i = 3$ gözlemlendi.
MLE $\hat{p} = 0.3$ verir. Elimizde ``$p$'nin $0.5$ civarında olduğunu bekliyoruz''
şeklinde bir ön bilgi varsa, bunu tahminde nasıl kullanırız?
Sonuç $0.3$'ten nasıl farklılaşır?
\kesifcevap{Önsel $\pi(p) = \mathrm{Beta}(5,5)$ ($p \approx 0.5$ sezgisi) seçilirse
  sonsal $p \mid \mathbf{x} \sim \mathrm{Beta}(5+3, 5+7) = \mathrm{Beta}(8,12)$;
  Bayes tahmini (kare kayıp) $= 8/20 = 0.4$. Veri az olduğundan önsel ağır çeker;
  $n \to \infty$ limitinde Bayes tahmini MLE'ye yaklaşır.}
\end{kesif}
...
\kendinleifadecevap{Önsel $\mathrm{Beta}(\alpha,\beta)$ + $k$ başarı, $n$ deneme $\Rightarrow$
  sonsal $\mathrm{Beta}(\alpha+k,\,\beta+n-k)$.
  Bayes tahmincisi (kare kayıp): $\hat{p}_B = \frac{\alpha+k}{\alpha+\beta+n}
  = \frac{n}{n+\alpha+\beta}\underbrace{\frac{k}{n}}_{\hat{p}_\mathrm{MLE}}
  + \frac{\alpha+\beta}{n+\alpha+\beta}\underbrace{\frac{\alpha}{\alpha+\beta}}_{\text{önsel ort.}}$.
  $\alpha+\beta$ "hayali örneklem": öncel inancın ağırlığı bu kadar gerçek gözleme eşdeğer.
  $n\to\infty$: $\hat{p}_B\to\hat{p}_\mathrm{MLE}$ --- veri önsel'i bunaltır.}
\end{kendinleifade}
...
%% ============================================================
\section{Tahmin Teorisinde Kayıp Fonksiyonları ve Risk}
\label{sec:kayip_risk}
%% ============================================================

\begin{kesif}
Bir tahmin edici ne zaman \emph{iyi}dir? Yansızlık tek ölçüt değildir. Karar teorisi, bir \emph{kayıp fonksiyonu} $L(\theta,d)$ aracılığıyla tahmin hatasının maliyetini sayısallaştırır.
\kesifcevap{Risk: $R(\theta,d)=E_\theta[L(\theta,d(X))]$.
  Kabul edilebilirlik: $d$'yi tüm $\theta$'da dominate eden $d'$ yoktur.
  Bayes risk minimizasyonu genellikle kabul edilebilir tahmin ediciler üretir.
  Stein paradoksu: $n\geq 3$'te $\bar{X}$ kare kayıpta kabul edilemezdir.}
\end{kesif}

\begin{tanim}[Kayıp fonksiyonu ve risk]\label{def:kayip_risk}
$\theta\in\Theta$ parametre, $d\in\mathcal{D}$ karar (tahmin). \emph{Kayıp fonksiyonu} $L:\Theta\times\mathcal{D}\to[0,\infty)$ hata maliyetini ölçer. Bir tahmin edici $\delta(X)$'in \emph{riski}:
\[
  R(\theta,\delta) = \Beklenti_\theta[L(\theta,\delta(X))].
\]
\end{tanim}

\begin{ornek}[Yaygın kayıp fonksiyonları]\label{ex:kayip_ornekleri}
\begin{enumerate}
  \item \textbf{Karesel kayıp} (\emph{squared error loss}): $L(\theta,d)=(\theta-d)^2$; risk = MSE $= \Var(\delta)+\text{bias}^2$.
  \item \textbf{Mutlak kayıp} (\emph{absolute error loss}): $L(\theta,d)=|\theta-d|$; optimal tahmin edici posterior medyanıdır.
  \item \textbf{0-1 kayıp}: $L(\theta,d)=\mathbf{1}[\theta\neq d]$; kesikli parametrelerde sık kullanılır; optimal tahmin edici moddur.
  \item \textbf{LINEX kayıp} (\emph{linear-exponential}): $L(\theta,d)=e^{a(\theta-d)}-a(\theta-d)-1$; asimetrik maliyet durumlarında.
\end{enumerate}
\end{ornek}

\begin{tanim}[MSE ayrışımı]\label{def:mse_ayrisim}
Karesel kayıp altında
\[
  \mathrm{MSE}(\delta) = \Beklenti_\theta[(\delta(X)-\theta)^2]
  = \Var_\theta(\delta(X)) + [\mathrm{bias}_\theta(\delta)]^2.
\]
\end{tanim}

\begin{teorem}[Yanlı-varyans dengesi]\label{thm:yanlı_varyans}
Küçük varyans ile küçük yanlılık aynı anda elde edilemeyebilir. MLE, $\hat\sigma^2_{\mathrm{MLE}}=\frac{1}{n}\sum(X_i-\bar X)^2$ örneğinde yanlı ama $\frac{n}{n-1}$ ile düzeltilince ($S^2$) varyans artar: $\mathrm{MSE}(\hat\sigma^2_{\mathrm{MLE}})<\mathrm{MSE}(S^2)$ tüm $n$ için.
\end{teorem}

\begin{proof}
$\Beklenti[\hat\sigma^2_{\mathrm{MLE}}]=\frac{n-1}{n}\sigma^2$ (yanlı), $\Var(\hat\sigma^2_{\mathrm{MLE}})=\frac{2(n-1)\sigma^4}{n^2}$.
$\mathrm{MSE}(\hat\sigma^2_{\mathrm{MLE}})=\frac{2(n-1)\sigma^4}{n^2}+\frac{\sigma^4}{n^2}=\frac{(2n-1)\sigma^4}{n^2}$.
$\mathrm{MSE}(S^2)=\Var(S^2)=\frac{2\sigma^4}{n-1}$.
$\frac{2n-1}{n^2}<\frac{2}{n-1}$ eşdeğerliği $(2n-1)(n-1)<2n^2$, yani $2n^2-3n+1<2n^2$, yani $-3n+1<0$: $n\geq1$ için doğru.
\end{proof}

\begin{hataanalizi}
"Yansız tahmin edici her zaman daha iyidir" yanlıştır. MSE kriteri altında küçük yanlılık + küçük varyans ikilisi, büyük varyans + sıfır yanlılıktan üstün olabilir. Özellikle regresyon ve Lasso gibi yöntemler bilinçli yanlılık ekleyerek MSE'yi düşürür.
\end{hataanalizi}

\begin{disiplinlerarasibaglan}
\textbf{Makine Öğrenmesi:} Ridge regresyon ve Lasso, yanlı tahmin edicilerdir; ancak yüksek boyutlu ortamda MSE'yi sıfır yanlılıklı OLS'den düşük tutar. Varyans-yanlılık dengesi ("bias-variance tradeoff") derin öğrenme optimizasyonunun temel kavramlarından biridir.

\textbf{Ekonometri:} LINEX kayıp, döviz kuru tahmininde asimetrik hata maliyetini modellemek için kullanılır: aşırı tahmin ile eksik tahminin maliyetleri farklıdır.
\end{disiplinlerarasibaglan}

%% ============================================================
\section{MLE'nin Varlığı, Tekliği ve Düzenlilik Koşulları}
\label{sec:mle_duzenlilk}
%% ============================================================

\begin{motivasyon}[MLE her zaman var mı?]
MLE'nin skor denklemi $\ell_n'(\theta)=0$'ın çözümü her zaman mevcut olmayabilir ya da birden fazla olabilir. Bu bölüm MLE'nin varlık ve teklik koşullarını inceler.
\end{motivasyon}

\begin{tanim}[Düzenlilik koşulları (Cramér)]\label{def:duzenlilik}
Aşağıdaki koşullar \emph{Cramér düzenlilik koşulları} olarak adlandırılır:
\begin{enumerate}
  \item[(R1)] Destek $\{x:f(x;\theta)>0\}$, $\theta$'ya bağlı değil.
  \item[(R2)] $\log f(x;\theta)$ ikinci kez türevlenebilir; türevler integrasyon sırası ile değiştirilebilir.
  \item[(R3)] Fisher bilgisi $0<\mathcal{I}(\theta)<\infty$.
  \item[(R4)] Skor fonksiyonu $\ell_n'(\theta)=0$'ın $\hat\theta_n$ tek çözümü vardır ve bu \emph{global maksimum}dur.
\end{enumerate}
\end{tanim}

\begin{teorem}[MLE tekliği --- üstel aile]\label{thm:mle_teklik}
$\{f(x;\theta):\theta\in\Theta\}$ tam düzenli üstel aile olsun. O zaman $\hat\theta_n^{\mathrm{MLE}}$ her $n$ için vardır, tektir ve Rao-Blackwell'e göre UMVUE ile çakışır.
\end{teorem}

\begin{proof}[Proof (taslak)]
Tam düzenli üstel ailede $\ell_n(\theta)=n\bar T\cdot\eta(\theta)-nA(\eta(\theta))$ (doğal parametreli). $A(\eta)$ (log-normalizing constant) $\eta$'da kesinlikle dışbükeydir: $A''(\eta)=\Var(T)>0$. Bu nedenle $\ell_n(\eta)$ kesinlikle içbükeydir ve skor denkleminin tek çözümü vardır. Yeterlilik + tamlık + Lehmann-Scheffé $\Rightarrow$ bu tahmin edici UMVUE'dir.
\end{proof}

\begin{ornek}[Üstel ailede MLE]\label{ex:ustel_mle}
\begin{enumerate}
  \item $X_i\iid\Binom(m,p)$: $\hat p_n=\bar X/m$ --- tek çözüm, UMVUE.
  \item $X_i\iid\Pois(\lambda)$: $\hat\lambda_n=\bar X$ --- tek çözüm, UMVUE.
  \item $X_i\iid\Normal(\mu,\sigma^2)$ ($\sigma^2$ bilinmiyor): $(\hat\mu_n,\hat\sigma^2_n)=(\bar X,\frac1n\sum(X_i-\bar X)^2)$ --- $\mu$ için UMVUE ama $\sigma^2$ için yanlı.
\end{enumerate}
\end{ornek}

\begin{kritiksorgu}
Düzenlilik koşulları (R1) ihlal edilirse MLE ne olur? Örnek: $X_i\iid\Uniform(0,\theta)$. Destek $\theta$'ya bağlı olduğundan skor denklemi anlamsız olur; MLE $\hat\theta_n=X_{(n)}$ (maksimum) ama normal teori geçerli değildir ve $n(\theta-X_{(n)})\dto\Exp(1/\theta)$ (üstel yakınsama).
\end{kritiksorgu}

\begin{ispatiskele}
\textbf{İspat taslağı: MLE tekliği --- üstel aile}
\begin{enumerate}
  \item $f(x;\theta)$'yı doğal parametreli üstel aile biçiminde yaz.
  \item Log-olabilirlik $\ell_n(\eta)$'nun içbükeyliğini $A''(\eta)=\Var_\eta(T)>0$ ile göster.
  \item Kesinlikle içbükey fonksiyonun en çok bir stasyoner noktası olduğunu belirt.
  \item Bu stasyoner noktanın global maksimum olduğunu sonuçlandır.
\end{enumerate}
\end{ispatiskele}

%% ============================================================
\section{Semiparametrik Etkinlik ve Etkili Skor}
\label{sec:semiparametrik}
%% ============================================================

Klasik CR alt sınırı, tam parametrik model varsayar. Semiparametrik istatistikte modelin bir kısmı bilinsiz bırakılır; bu durumda CR sınırı genişler.

\begin{tanim}[Etkili skor]\label{def:etkili_skor}
Parametrik model $\{p_{\theta,\eta}:\theta\in\mathbb{R},\,\eta\in\mathcal{H}\}$; $\theta$ ilgi parametresi, $\eta$ bozulma parametresi (sonsuz boyutlu olabilir). Etkili skor $\tilde\ell_\theta = \ell_\theta - \Pi(\ell_\theta|\mathcal{T})$; burada $\Pi$ projeksiyon $\ell_\eta$'nın oluşturduğu $\mathcal{T}$ uzayına.
\end{tanim}

\begin{teorem}[Semiparametrik CR alt sınırı]\label{thm:semi_cr}
$\tilde I(\theta)=E[\tilde\ell_\theta^2]$ etkili Fisher bilgisi. $\theta$'nın yansız tahmincisi için:
\[
  \Var(\hat\theta) \geq \frac{1}{n\tilde I(\theta)}.
\]
$\tilde I(\theta)\leq I(\theta)$: semiparametrik sınır, parametrik sınırdan daha zayıf (daha geniş).
\end{teorem}

\begin{ornek}[Konum-ölçek modeli]\label{ex:konum_olcek}
$X_i=\theta+\sigma\varepsilon_i$ ($\varepsilon_i$ bilinmeyen, simetrik yoğunluklu). Konum $\theta$ için etkili skor hesaplanır: $\tilde\ell_\theta=-f'(x-\theta)/f(x-\theta)$ (skor orijini). Kapalı form: $\tilde I(\theta)=\int[f'(x)/f(x)]^2f(x)dx$ --- bu tam olarak $f$ biliniyorsa parametrik bilgidir. Bilinmiyorsa $\tilde I<I$ --- belirsizlik kaybı.
\end{ornek}

\subsection{Adaptif Tahmin}

\begin{tanim}[Adaptif tahmin edici]\label{def:adaptif}
$\hat\theta_n$ adaptiftir (\emph{adaptive}): $\sqrt{n}(\hat\theta_n-\theta)\xrightarrow{d}\Normal(0,1/I(\theta))$ --- yani bozulma parametresi bilinmeseydi de aynı sınıra ulaşılır.
\end{tanim}

Stein (1956): simetrik yoğunluklar için adaptif tahmin edici mevcuttur. Bickel (1982): konum ailesi için kaçınılmaz koşullar.

\begin{teorem}[Adaptivite koşulu]\label{thm:adaptivite}
$f$ simetrik ve iki kez türevlenebilir olsun. Konum parametresi için adaptif tahmin edici mevcuttur $\iff$ $f$ belirli düzgünlük koşullarını sağlar (tipik olarak: $f\in C^2$, kuyruğun davranışı düzenli).
\end{teorem}

\noindent Bu, semiparametrik teorinin en çarpıcı sonuçlarından biridir: bilinmeyen hata dağılımı, asimptotik olarak hiçbir kayba yol açmaz.
...
\begin{itemize}
  ...
  \item Kayıp fonksiyonu + risk: MSE = varyans + yanlılık$^2$; yanlı tahmin edici daha düşük MSE verebilir.
  \item Üstel ailede MLE tekdir ve UMVUE ile çakışır; düzenlilik koşulları ihlalinde MLE farklı davranır.
\end{itemize}
\end{mikroozet}
...
\begin{alistirma}[Al.12.2b]{B}{Rao-Blackwell Adımı: Poisson}
$X_1,\ldots,X_n\iid\Pois(\lambda)$. $\delta(X_1)=\mathbf{1}[X_1=0]=e^{-\lambda}$'nın yansız tahmincisi. $T=\sum X_i$ yeterli. (a) $\delta_{\mathrm{RB}}(T)=E[\delta(X_1)|T=t]$'yi hesapla. (b) Bu tahmin edici neden $\delta(X_1)$'den daha iyidir?
\end{alistirma}
\alcozum{%
(a) $E[\mathbf{1}[X_1=0]|T=t]=P(X_1=0|T=t)$. $T=t$ koşulunda $X_1\sim\Binom(t,1/n)$ (Poisson birleşimi ayrışımı):
$P(X_1=0|T=t)=(1-1/n)^t$.
$\delta_\mathrm{RB}(T)=(1-1/n)^T$ --- UMVUE (tamamlayıcı yeterli $T$'nin fonksiyonu, yansız).
(b) Rao-Blackwell: $\mathrm{Var}(\delta_\mathrm{RB})\leq\mathrm{Var}(\delta)$. $\delta(X_1)$ sadece $X_1$ kullanırken $\delta_\mathrm{RB}$ tüm $T$'yi kullanır.}

\begin{alistirma}[Al.12.2c]{B}{MSE Karşılaştırması: MLE vs Yansız}
$X_1,\ldots,X_n\iid\Normal(\mu,\sigma^2)$. (a) $\hat\sigma^2_{\mathrm{MLE}}=\frac{1}{n}\sum(X_i-\bar X)^2$'nin MSE'sini hesapla. (b) $S^2=\frac{1}{n-1}\sum(X_i-\bar X)^2$'nin MSE'sini hesapla. (c) Hangisi daha küçük MSE'ye sahip?
\end{alistirma}
\alcozum{%
$(n-1)S^2/\sigma^2\sim\chi^2_{n-1}$: $E[S^2]=\sigma^2$, $\Var(S^2)=2\sigma^4/(n-1)$.
(a) $\hat\sigma^2_\mathrm{MLE}=\frac{n-1}{n}S^2$: $E[\hat\sigma^2_\mathrm{MLE}]=\frac{n-1}{n}\sigma^2$ (yanlı). $\mathrm{MSE}(\hat\sigma^2_\mathrm{MLE})=\Var+\mathrm{bias}^2=\frac{(n-1)^2}{n^2}\cdot\frac{2\sigma^4}{n-1}+\frac{\sigma^4}{n^2}=\frac{(2n-1)\sigma^4}{n^2}$.
(b) $\mathrm{MSE}(S^2)=\Var(S^2)=\frac{2\sigma^4}{n-1}$.
(c) $\frac{2n-1}{n^2}<\frac{2}{n-1}$: her $n\geq1$ için $\hat\sigma^2_\mathrm{MLE}$ daha küçük MSE'ye sahip.}
