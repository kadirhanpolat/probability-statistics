...
\kesifcevap{$H_0:m=m_0$ altında $S=\sum\mathbf{1}(X_i>m_0)\sim\mathrm{Binom}(n,1/2)$.
  Test: $p=2\min(P(S\leq s_\text{obs}),P(S\geq s_\text{obs}))$ iki yönlü.
  Güç düşük ama tam dağılımdan bağımsız (distribution-free) ve süreklilik gerekmez.}
\end{kesif}
...
\begin{kesif}
Bir tahmin edicinin standart hatasını analitik hesaplayamıyorsak
(örn.\ medyanın SE'si, oranların oranı) ne yapabiliriz?
Elimizdeki tek kaynak $n$ gözlemden oluşan bir veri kümesiyse,
örnekleme dağılımını nasıl yaklaşık hesaplarız?
\kesifcevap{Nonparametrik bootstrap: elimizdeki $n$ gözlemi ``popülasyon'' gibi
  kabul edip geri koyarak $B$ kez yeniden örnekle; her tekrarda $T^*$ hesapla.
  $T^*$'ların dağılımı, gerçek $T$'nin örnekleme dağılımını yaklaşıklar.
  $B \geq 1000$ standart için yeterli; BCa güven aralığı çarpıklığı düzeltir.}
\end{kesif}
...
\begin{kesif}
Bir histogramda kutu genişliği $h$ küçükse tahmin pürüzlü ve gürültülü;
$h$ büyükse pürüzsüz ama ayrıntılar kaybolur. Her gözlem noktasını merkez
kabul eden ve $h$ ile pürüzlülüğü kontrol edilen sürekli bir yoğunluk tahmincisi
nasıl inşa edilebilir?
\kesifcevap{Her $X_i$ çevresine $h$ bant genişliğiyle simetrik bir ``çekirdek''
  $K(\cdot)$ yerleştir ve topla:
  $\hat{f}_h(x) = \frac{1}{nh}\sum_{i=1}^n K\!\bigl((x-X_i)/h\bigr)$.
  Küçük $h$: her gözlem sivri bir tepe; büyük $h$: tepeler birleşir, ayrıntı kaybolur.
  Optimal $h$: Silverman kuralı $h=1.06\hat\sigma n^{-1/5}$ (Normal çekirdek).}
\end{kesif}
...
\begin{hataanalizi}
Theil-Sen'i EKK ile karıştırmayınız: EKK aykırı değerlere duyarlıdır (her noktanın etkisi kareli hataya orantılı). Theil-Sen'de bir aykırı değer en fazla $n-1$ eğim çiftini etkiler ve medyan sağlamlığı korur. Veri aykırı değersiz ve normal dağılımlıysa EKK hafif daha verimlidir.
\end{hataanalizi}
...
(a) Rütbeleri hesaplayınız (bağ yok). (b) $W^+$ ve $W^-$'yı bulunuz. (c) $\alpha=0.05$ için $H_0$'ı test ediniz (çift yönlü, tablo kritik değeri $W_{0.025,8}=4$).
\end{alistirma}
\alcozum{%
...
(a) Tüm $N=12$ değeri birleştirerek rütbele. (b) Her grubun rütbe toplamını bulunuz. (c) $H$ istatistiğini hesaplayınız ve $\alpha=0.05$ için karar veriniz ($\chi^2_{2,0.05}=5.99$).
\end{alistirma}
\alcozum{%
...
$n=15$ gözlem, $H_0:\tilde\mu=10$ vs $H_1:\tilde\mu>10$. Gözlemlerden 11'i 10'un üzerinde. (a) Test istatistiğini belirtiniz. (b) $p$-değerini Binom$(15,0.5)$ kullanarak hesaplayınız.
\end{alistirma}
\alcozum{%
...
İki bağımsız grup: $X$: $\{3,7,5,11,9\}$, $Y$: $\{1,4,8,2,6\}$. $H_0$: iki grup dağılımı aynı. Wilcoxon-Mann-Whitney istatistiğini hesaplayınız ve $n_1=n_2=5$ için kritik değerle karşılaştırınız.
\end{alistirma}
\alcozum{%
