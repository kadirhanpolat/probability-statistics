...
\kesifcevap{Nonparametrik bootstrap: $\hat{F}_n$'den geri koyarak $B$ tekrar örnekle.
  Her tekrarda $T^*=T(X_1^*,\ldots,X_n^*)$ hesapla; kantilleri GA için kullan.
  Parametrik bootstrap: $\hat\theta$'dan model simüle eder.
  Kural: $B\geq 1000$ yeterli; BCa (bias-corrected) CI daha güvenilir.}
\end{kesif}
...
\kesifcevap{$\hat{f}_h(x)=\frac{1}{nh}\sum_{i=1}^n K\!\left(\frac{x-X_i}{h}\right)$.
  Bant genişliği $h$: küçük $\to$ yüksek varyans, büyük $\to$ aşırı düzleştirme.
  Silverman kuralı: $h=1.06\hat\sigma n^{-1/5}$ Normal çekirdeği için optimal.
  $L_2$ risk minimizasyonu cross-validation ile $h$ seçimini verir.}
\end{kesif}
