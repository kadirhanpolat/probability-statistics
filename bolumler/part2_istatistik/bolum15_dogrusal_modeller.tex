...
\kendinleifadecevap{Ridge = MAP ile Normal önsel:
  $\beta_j \overset{\text{iid}}{\sim} N(0,\,\sigma^2/\lambda)$, log-sonsal
  minimizasyonu $\|\mathbf{y}-\mathbf{X}\boldsymbol\beta\|^2+\lambda\|\boldsymbol\beta\|^2$'yi verir.
  Lasso = Laplace (çift üstel) önsel: $\beta_j \overset{\text{iid}}{\sim}
  \mathrm{Laplace}(0,1/\lambda)$, log-sonsal $L_1$ cezayı doğurur.
  Sezgi: Normal önsel sıfıra büzer ama sıfırlamaz; Laplace önsel köşe noktasında
  tam sıfırlamayı tercih eder --- değişken seçimi.}
\end{kendinleifade}
...
\begin{kesif}
$Y_i \in \{0,1\}$ ikili yanıt için doğrusal model
$E[Y_i \mid \mathbf{x}_i] = \mathbf{x}_i^\top\boldsymbol\beta$ kullanılabilir mi?
$\mathbf{x}_i^\top\boldsymbol\beta < 0$ veya $> 1$ olduğunda ne sorun çıkar?
Olasılıkları $[0,1]$ aralığında tutmak için ne yapılabilir?
\kesifcevap{Doğrusal model $\hat{p}_i = \mathbf{x}_i^\top\boldsymbol\beta < 0$ veya $> 1$
  üretebilir --- olasılık yorumu bozulur. Çözüm: $E[Y_i]$'yi doğrudan değil,
  bir dönüşümünü modellemek. $g(\mu) = \log(\mu/(1-\mu))$ (logit) bağlantısı
  $\mu \in (0,1)$'i tüm gerçel eksene açar; bu lojistik regresyondur.
  Sayma verisi için $g = \log$ ile $\hat\mu > 0$ garantisi sağlanır.}
\end{kesif}
...
%% ============================================================
\section{Doğrusal Modelde Tahmin Bölgesi ve Eşzamanlı Çıkarım}
\label{sec:tahmibolgesi}
%% ============================================================

\begin{tanim}[Ortalama yanıt güven bandı]\label{def:guven_bandi}
$\mathbf{x}_0^*=(1,x_{01},\ldots,x_{0,p-1})^\top$ verilen bir kovaryat vektörü. $E[Y|\mathbf{x}_0]=\mathbf{x}_0^\top\boldsymbol\beta$ için $(1-\alpha)$ güven aralığı:
\[
  \hat Y_0 \pm t_{n-p,\alpha/2}\cdot S_e\sqrt{\mathbf{x}_0^\top(\mathbf{X}^\top\mathbf{X})^{-1}\mathbf{x}_0}.
\]
\end{tanim}

\begin{tanim}[Yeni gözlem tahmin bandı]\label{def:tahmin_bandi}
Yeni $Y_0=\mathbf{x}_0^\top\boldsymbol\beta+\varepsilon_0$ için $(1-\alpha)$ tahmin aralığı:
\[
  \hat Y_0 \pm t_{n-p,\alpha/2}\cdot S_e\sqrt{1+\mathbf{x}_0^\top(\mathbf{X}^\top\mathbf{X})^{-1}\mathbf{x}_0}.
\]
Güven aralığından $S_e^2$ (yeni gözlem belirsizliği) kadar geniş.
\end{tanim}

\begin{teorem}[Scheffe eşzamanlı güven bandı]\label{thm:scheffe_guven}
Tüm $\mathbf{x}_0\in\mathbb{R}^p$ için eşzamanlı $(1-\alpha)$ güven bandı:
\[
  \hat Y_0 \pm \sqrt{p\cdot F_{p,n-p,\alpha}}\cdot S_e\sqrt{\mathbf{x}_0^\top(\mathbf{X}^\top\mathbf{X})^{-1}\mathbf{x}_0}.
\]
Ayrı ayrı $t$ bantlarından geniştir; her $\mathbf{x}_0$ için eşzamanlı kapsama garantisi sunar.
\end{teorem}

\begin{proof}[Taslak]
$\mathbf{c}^\top\hat{\boldsymbol\beta}-\mathbf{c}^\top\boldsymbol\beta$ için tüm $\mathbf{c}$ üzerindeki uniform supremum $F_{p,n-p}$ dağılımına oturur; $F=t^2/p$ bağlantısı ile Scheffe kritik değeri türetilir.
\end{proof}

\begin{ornek}[Regresyon güven ve tahmin bandı karşılaştırması]
$n=30$, $p=2$, $S_e=2.5$; $\mathbf{x}_0=(1,5)^\top$, $\mathbf{x}_0^\top(\mathbf{X}^\top\mathbf{X})^{-1}\mathbf{x}_0=0.08$; $\hat Y_0=12$.
\begin{itemize}
  \item Güven bandı (ortalama): $12\pm2.048\cdot2.5\sqrt{0.08}=12\pm1.45$; $[10.55,\;13.45]$.
  \item Tahmin bandı (yeni): $12\pm2.048\cdot2.5\sqrt{1.08}=12\pm5.32$; $[6.68,\;17.32]$.
  \item Scheffe ($p=2$): $12\pm\sqrt{2\cdot3.35}\cdot2.5\sqrt{0.08}=12\pm2.58\cdot0.707=12\pm1.82$; $[10.18,\;13.82]$.
\end{itemize}
Tahmin bandı çok daha geniş (bireysel belirsizlik). Scheffe, standart $t$-bandından $2.58/2.048=1.26$ kat geniş --- eşzamanlılık maliyeti.
\end{ornek}

%% ============================================================
\section{Hiyerarşik ve Karışık Modeller: Giriş}
\label{sec:hiyerarsik_model}
%% ============================================================

\begin{tanim}[Karışık lineer model]\label{def:karisik_model}
$\mathbf{Y}=\mathbf{X}\boldsymbol\beta+\mathbf{Z}\mathbf{b}+\boldsymbol\varepsilon$; $\mathbf{b}\sim\Normal(0,G)$ rasgele etki, $\boldsymbol\varepsilon\sim\Normal(0,R)$. Marjinal dağılım: $\mathbf{Y}\sim\Normal(\mathbf{X}\boldsymbol\beta,\mathbf{Z}G\mathbf{Z}^\top+R)=\Normal(\mathbf{X}\boldsymbol\beta,V)$.
\end{tanim}

\begin{teorem}[BLUE ve BLUP]\label{thm:blup}
Sabit etki $\boldsymbol\beta$ için \emph{en iyi doğrusal yansız tahminci} (BLUE):
\[
  \hat{\boldsymbol\beta}_{\mathrm{GLS}} = (\mathbf{X}^\top V^{-1}\mathbf{X})^{-1}\mathbf{X}^\top V^{-1}\mathbf{Y}.
\]
Rasgele etki $\mathbf{b}$ için \emph{en iyi doğrusal yansız öngörü} (BLUP):
\[
  \hat{\mathbf{b}} = G\mathbf{Z}^\top V^{-1}(\mathbf{Y}-\mathbf{X}\hat{\boldsymbol\beta}).
\]
\end{teorem}

\begin{ornek}[İki seviyeli hiyerarşik model]
$Y_{ij}=\mu+u_i+\varepsilon_{ij}$; $u_i\iid\Normal(0,\sigma_u^2)$ okul etkisi, $\varepsilon_{ij}\iid\Normal(0,\sigma^2)$ öğrenci hatası. $k=10$ okul, her okuldan $n_i=20$ öğrenci. $V=\sigma_u^2\mathbf{1}\mathbf{1}^\top+\sigma^2 I$. REML ile $\hat\sigma_u^2$, $\hat\sigma^2$ tahmini; ICC$=\hat\sigma_u^2/(\hat\sigma_u^2+\hat\sigma^2)$.
\end{ornek}
...
$Y_i=\alpha+\beta x_i+\varepsilon_i$, $\varepsilon_i\iid\Normal(0,\sigma^2)$. (a) Log-olabilirlik $\ell(\alpha,\beta,\sigma^2)$'yi yaz. (b) $\partial\ell/\partial\alpha=0$, $\partial\ell/\partial\beta=0$ denklemlerini çözerek $\hat\alpha,\hat\beta$ EKK formüllerine ulaşınız.
\end{alistirma}
\alcozum{%
...
$Y\sim\mathrm{Bernoulli}(p)$, $\log\frac{p}{1-p}=\beta_0+\beta_1 x$. $n=5$ gözlem: $(x,y)$: $(0,0),(1,0),(2,1),(3,1),(4,1)$. (a) Log-olabilirliği yaz. (b) $\beta_0=0,\beta_1=1$ başlangıcından Newton adımı için skor $\mathbf{s}$ ve Hessian $\mathbf{H}$'yi numerik hesaplayınız.
\end{alistirma}
\alcozum{%
