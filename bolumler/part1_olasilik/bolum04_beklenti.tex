...
\kesifcevap{Lebesgue integrali tüm durumları kapsar: $E[X]=\int_\Omega X\,dP$.
  Ayrık: $\sum_x x P(X=x)$; sürekli: $\int_{-\infty}^\infty x f(x)\,dx$;
  karma: $\int x\,dF_X(x)$ (Lebesgue-Stieltjes). Tek tanım, üç görünüm.}
\end{kesif}
...
integral inşasını hatırlayınız. Beklenti, olasılık ölçüsüne göre bu
...
\kesifcevap{MGF: $M_X(t)=E[e^{tX}]$; karakteristik fonksiyon: $\varphi_X(t)=E[e^{itX}]$.
  $M_X^{(k)}(0)=E[X^k]$ ile tüm momentler kodlanır. Karakteristik fonksiyon
  her zaman var olur; Lévy ters dönüşüm teoremi ile dağılımı biricik belirler.
  İki rasgele değişkenin KF'leri eşitse dağılımları özdeştir.}
\end{kesif}
...
\begin{kesif}
$Y = 2X + \varepsilon$ ($\varepsilon \perp X$) modelini düşününüz.
...
\kesifcevap{$E[Y|\mathcal{G}]$ --- $\mathcal{G}$-ölçülebilir ve $L^2$-dik izdüşümü:
  $E[(Y-E[Y|\mathcal{G}])Z]=0$ her $\mathcal{G}$-ölçülebilir $Z$ için.
  $\mathcal{G}=\sigma(X)$ seçilince $E[Y|X=x]$ klasik tanıma döner.
  Bu soyutlama martingal teorisi ve filtrasyon yapısının temelidir.}
\end{kesif}
...
\begin{kendinleifade}
Koşullu beklentinin "bilgiyi güncellemek" yorumunu kendi cümlelerinizle açıklayınız.
"Kule özelliği" ne anlama gelir? Somut bir örnek üretiniz.
\kendinleifadecevap{$E[Y|\mathcal{G}]$: "$\mathcal{G}$ bilgisi verilince $Y$ için en iyi tahmin."
  Daha fazla bilgi ($\mathcal{G}_1\subset\mathcal{G}_2$) tahmini iyileştirir.
  \textbf{Kule özelliği:} $E\bigl[E[Y|\mathcal{G}_2]\bigm|\mathcal{G}_1\bigr]=E[Y|\mathcal{G}_1]$
  ($\mathcal{G}_1\subset\mathcal{G}_2$) --- önce geniş bilgiyle, sonra dar bilgiyle ortalama almak,
  direkt dar bilgiyle ortalama almaya eşit.
  \textbf{Örnek:} $Y$=sınav notu; $\mathcal{G}_2=\sigma(\text{öğrenci})$, $\mathcal{G}_1=\sigma(\text{bölüm})$.
  Öğrenci ortalamasının bölüm ortalaması = direkt bölüm ortalaması.}
\end{kendinleifade}
...
$E[X]$ ve $E[X^2]$'yi hesaplayınız. Ardından $\mathrm{Var}(X)$'i bulunuz.
\end{alistirma}
\alcozum{%
...
(a) $E[\ln X]$'i en fazla kaç yapabilirsiniz? Üst sınırı bulunuz.
(b) $E[1/X]$'in alt sınırını Jensen ile bulunuz.
\end{alistirma}
\alcozum{%
...
(a) $M_X(t) = e^{\lambda(e^t-1)}$ olduğunu gösteriniz.
(b) $M_X'(0)$ ve $M_X''(0)$ ile $E[X]$ ve $E[X^2]$'yi bulunuz.
(c) $\mathrm{Var}(X)$'i hesaplayınız.
\end{alistirma}
\alcozum{%
...
(a) $E[Y \mid X = x]$'i bulunuz.
(b) $E[Y]$'yi kule özelliğiyle hesaplayınız.
(c) $\mathrm{Var}(Y)$'yi $\mathrm{Var}(Y) = E[\mathrm{Var}(Y\mid X)] + \mathrm{Var}(E[Y\mid X])$ ile bulunuz.
\end{alistirma}
\alcozum{%
(a) $E[Y \mid X=x] = x/2$.\\
...
(a) $\varphi_{X+Y}(t)=\varphi_X(t)\varphi_Y(t)$ olduğunu gösteriniz.
(b) $X,Y\iid\Normal(0,1)$: $\varphi_X(t)=e^{-t^2/2}$ kullanarak $Z=X+Y$ dağılımını bulunuz.
(c) $X\sim\Pois(\lambda_1)$, $Y\sim\Pois(\lambda_2)$ bağımsız: $Z=X+Y$ dağılımını KF yoluyla belirleyiniz.
\end{alistirma}
\alcozum{%
...
(a) Marjinal yoğunlukları $f_X(x)$ ve $f_Y(y)$'yi bulunuz.
(b) $X$ ve $Y$ bağımsız mı?
(c) $\Cov(X,Y)$, $\Cor(X,Y)$'yi hesaplayınız.
(d) $E[Y\mid X=0.5]$'i bulunuz. Kule özelliğiyle $E[Y]$'yi doğrulayınız.
\end{alistirma}
\alcozum{%
...
$E[(2X-1)^2]$ ve $\mathrm{Var}(3X+5)$'i hesaplayınız.
\end{alistirma}
\alcozum{%
...
(a) $P(|X - 50| \geq 10)$'un Chebyshev üst sınırını bulunuz.
(b) $P(40 \leq X \leq 60) \geq ?$ alt sınırını veriniz.
\end{alistirma}
\alcozum{%
...
$X \geq 0$, $E[X] = \mu$.
(a) $P(X \geq k\mu) \leq 1/k$ olduğunu gösteriniz.
(b) $P(X \geq 2\mu) = 1/2$ sağlayan bir dağılım inşa ediniz (Markov sıkılığı).
\end{alistirma}
\alcozum{%
...
$\mathrm{Var}(X + Y + Z)$'i hesaplayınız.
\end{alistirma}
\alcozum{%
...
(a) $\varphi_X(t) = 1 - p + pe^{it}$ olduğunu gösteriniz.
(b) $X_1, \ldots, X_n$ bağımsız $\mathrm{Bernoulli}(p)$; $S_n = \sum X_i$.
$\varphi_{S_n}(t)$'yi bulunuz ve bunun hangi dağılıma ait olduğunu tanıyınız.
\end{alistirma}
\alcozum{%
...
(a) $E[S_N]$'i bulunuz. (b) $\mathrm{Var}(S_N)$'i bulunuz.
\end{alistirma}
\alcozum{%
...
$X, Y \in L^2(\Omega, \mathcal{F}, P)$ için $(E[XY])^2 \leq E[X^2] E[Y^2]$ olduğunu ispatlayınız.
...
(a) Chebyshev ile $P(|X - 2| \geq 2)$ için üst sınır veriniz.
(b) Cantelli ile $P(X - 2 \geq 2)$ için üst sınır veriniz.
...
(a) Hoeffding eşitsizliği ile $P(\bar{X}_{100} \geq 0.6)$ için üst sınır veriniz.
(b) Chebyshev ile (varyanslı $\sigma^2 = 1/12$ alarak) karşılaştırınız.
(c) $P \leq 0.01$ için gereken minimum $n$'yi Hoeffding ile bulunuz ($\varepsilon = 0.1$).
\end{alistirma}
\alcozum{%
...
(a) $E[X^3]$ ve $E[X^4]$'ü hesaplayınız ($E[X^k] = k!$ Üstel için).
(b) Çarpıklık $\gamma_1 = (E[(X-\mu)^3])/\sigma^3$'ü hesaplayınız.
(c) Basıklık fazlasını $\gamma_2 = E[(X-\mu)^4]/\sigma^4 - 3$ hesaplayınız.
\end{alistirma}
\alcozum{%
