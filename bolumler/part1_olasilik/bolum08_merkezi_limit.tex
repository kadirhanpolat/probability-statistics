...
(a) $\sqrt{n}(\mathbf{T}_n-\boldsymbol\theta)$ ($\boldsymbol\theta=(\mu,\sigma^2)$) için asimptotik kovaryans matrisini bulun.
(b) $g(\mu,\sigma^2)=\mu/\sigma$ (varyasyon katsayısı) için çok boyutlu delta yöntemini uygulayın.
(c) Elde ettiğiniz asimptotik varyansı yorumlayın.
\end{alistirma}
\alcozum{%
...
(a) $\bar X_n$'nin karakteristik fonksiyonu $\varphi_{\bar X_n}(t)=\varphi(t/n)^n$ olduğunu gösterin.
(b) $E[X_1]=\mu$, $\Var(X_1)=\sigma^2<\infty$ varsayımında $\log\varphi(t)\approx i\mu t - \sigma^2t^2/2+o(t^2)$ açılımını kullanarak $\sqrt{n}(\bar X_n-\mu)/\sigma\xrightarrow{d}\Normal(0,1)$'i türetin.
