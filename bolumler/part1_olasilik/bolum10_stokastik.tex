...
\begin{kesif}
Basit rasgele yürüyüşte $S_{n+1}$'in dağılımı, tüm geçmiş $S_0,S_1,\ldots,S_n$
yerine yalnızca $S_n$'e bağlıdır. Bu ``geçmişsizlik'' özelliğini koşullu
olasılık notasyonuyla nasıl yazarsınız?
\kesifcevap{$P(X_{n+1}=j \mid X_0=i_0,\ldots,X_n=i) = P(X_{n+1}=j \mid X_n=i)$.
  Tüm geçmiş bilgisi tek bir sayıya, mevcut duruma $X_n=i$, indirgenebilir.
  Geçiş olasılıkları $p_{ij}$ bu bilgiyi taşır; geçmişin ayrıntısı gereksizdir.}
\end{kesif}
...
\begin{kesif}
Riemann-Stieltjes toplamında bölüm noktası $t^*_k \in [t_k, t_{k+1}]$ serbestçe
seçilir ve klasik analizde seçim limiti etkilemez. $W$ Brownian hareketi için
$t^*_k = t_k$ (sol uç) ile $t^*_k = t_{k+1}$ (sağ uç) seçimlerinin aynı limiti
verip vermeyeceğini düşününüz.
\kesifcevap{Hayır --- Brownian artışlar $W_{t_{k+1}}-W_{t_k}$ değerlendirme
  noktası $t^*_k$'ya bağımsız değildir. Sol uç ($t^*_k=t_k$) Itô integralini,
  sağ uç Stratonovich integralini tanımlar. Somut fark:
  $\int_0^T W_s\,dW_s = W_T^2/2 - T/2$ (Itô) vs.\ $W_T^2/2$ (Stratonovich).
  Sol uç seçimi martingal özelliğini korur; bu istatistiksel uygulamalarda
  standart tercihdir.}
\end{kesif}
...
\begin{hataanalizi}
OU süreci "ortalamaya dönüş" gösterir --- başlangıçtan uzaklaştığında bir çekme kuvveti etkiler. Brownian hareket ile karıştırmayınız: $W(t)$ sınırsız büyür ($\Var(W(t))=t\to\infty$), OU ise durağan dağılıma yakınsar.
\end{hataanalizi}
...
\kesifcevap{Metropolis-Hastings: öneri $q(y|x)$'den aday $y$ üret;
  $\alpha=\min\!\left(1,\tfrac{\pi(y)q(x|y)}{\pi(x)q(y|x)}\right)$ oranıyla kabul et.
  Normalleştirme sabitine gerek kalmaz ($\pi$ çarpımsal olarak bilinmesi yeterli).
  Zincir ayrıntılı denge ($\pi_{ij}=\pi_{ji}$) koşuluyla $\pi$'ye yakınsar.}
\end{kesif}
...
$(X,Y)\sim\Normal_2(\mathbf{0},\boldsymbol\Sigma)$, $\boldsymbol\Sigma=\begin{pmatrix}1&\rho\\\rho&1\end{pmatrix}$. Gibbs adımlarının geçiş dağılımlarını yazınız. Neden durağan dağılım $\Normal_2(\mathbf{0},\boldsymbol\Sigma)$'dır?
\end{alistirma}
\alcozum{%
