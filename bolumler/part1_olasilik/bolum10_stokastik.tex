...
\kesifcevap{Markov özelliği: $P(X_{n+1}=j\mid X_0,\ldots,X_n)=P(X_{n+1}=j\mid X_n)=P_{ij}$.
  Geçiş matrisi $P=(P_{ij})$ tüm dinamiği belirler.
  Geçmiş bilgisi yalnızca mevcut durum üzerinden etkilidir; gereksiz bilgi depolanmaz.}
\end{kesif}
...
\kesifcevap{Brownian yollar n.k.\ her yerde türevsizdir (Hölder $<1/2$).
  Itô integrali: $\int_0^T H_s\,dW_s = L^2\text{-}\lim \sum H_{t_k}(W_{t_{k+1}}-W_{t_k})$
  (sol uç yaklaşımı). Riemann-Stieltjes'ten farkı: ikinci mertebe terim sıfırlanmaz;
  bu Itô formülünde $\frac{1}{2}f''(X_t)\,dt$ düzeltmesini doğurur.}
\end{kesif}
