...
\iskeleadim{1}{$P(X \leq x) = P(F^{-1}(U) \leq x)$ ifadesini yazınız.}
\iskeleadim{2}{$F^{-1}(u) \leq x \Leftrightarrow u \leq F(x)$ eşdeğerliğini kullanınız ($F$ sağdan sürekli).}
...
\begin{enumerate}[(1)]
  \item Ters dönüşümü bulunuz: $x_i = h_i(y_1,\ldots,y_n)$.
  \item Jacobian matrisini oluşturunuz: $J_{ij} = \partial h_i/\partial y_j$.
  \item $|\det J|$'yi hesaplayınız.
  \item Tanım bölgesini dönüştürünüz: $\mathcal{X} \to \mathcal{Y}$.
  \item $f_Y(\mathbf{y}) = f_X(h(\mathbf{y})) \cdot |\det J|$ ile PDF'yi yazınız.
\end{enumerate}
\end{onerme}
...
\begin{itemize}
  ...
  \item Sıra istatistiklerinin neden bağımlı olduğunu kendi cümlelerinizle açıklayınız.
  \item Delta yönteminin temel fikri nedir? Taylor açılımının bu bağlamda nasıl çalıştığını anlatınız.
...
(a) $f_Y(y)$'yi Jacobian yöntemiyle bulunuz.
(b) $E[Y]$'yi $f_Y$ üzerinden hesaplayınız ($\Gamma(3/2) = \sqrt{\pi}/2$).
\end{alistirma}
\alcozum{%
...
(a) $X_{(1)}$ ve $X_{(3)}$'ün PDF'lerini yazınız.
(b) $E[X_{(1)}]$ ve $E[X_{(3)}]$'ü hesaplayınız.
(c) $P(X_{(2)} > 0.5)$'i bulunuz.
\end{alistirma}
\alcozum{%
...
(a) $M_{S_n}(t)$'yi bulunuz.
(b) $S_n$'nin dağılımını belirtiniz.
(c) $\bar{X}_n = S_n/n$'nin MÜF'ünü yazınız ve $n=100$, $\lambda=2$ için
$P(\bar{X}_{100} \geq 2.5)$'e Chernoff sınırı veriniz.
\end{alistirma}
\alcozum{%
...
Delta yöntemi ile $\sqrt{n}(1/\bar{X}_n - 1/\mu)$'nun limit dağılımını bulunuz.
\end{alistirma}
\alcozum{%
...
(a) $(R,\Theta)$'nın birleşik yoğunluğunu Jacobian yöntemiyle bulunuz.
(b) $R$ ve $\Theta$'nın marjinal dağılımlarını yazınız. $R$'nin adı ne?
(c) Bu sonucu Box-Muller dönüşümüyle ilişkilendiriniz.
\end{alistirma}
\alcozum{%
...
(a) MÜF yöntemiyle $Z=X+Y$'nin dağılımını bulunuz.
(b) $W=X/(X+Y)$'nin dağılımını konvolüsyon + Jacobian ile türetiniz.
(c) $\alpha=\beta=1$ durumunda (yani $X,Y\iid\mathrm{Exp}(1)$): $W$'nin dağılımını yazınız ve sezgisel yorumunu veriniz.
\end{alistirma}
\alcozum{%
...
(a) $F_Y(y)$'yi $\Phi$ cinsinden yazınız. (b) $f_Y(y)$'yi türetiniz. (c) $P(1 < Y < e)$'yi bulunuz.
\end{alistirma}
\alcozum{%
...
(a) $X_{(1)}$'in dağılımını bulunuz. (b) $P(X_{(1)} > 0.3)$'ü hesaplayınız.
(c) $E[X_{(1)}]$'i bulunuz.
\end{alistirma}
\alcozum{%
...
(a) $(U,V)$'nin ortak PDF'sini bulunuz.
(b) $U$ ve $V$ bağımsız mı? Dağılımlarını belirtiniz.
(c) $U$ ve $V$'nin $\mathrm{Cov}(U,V)$'sini hesaplayınız.
\end{alistirma}
\alcozum{%
...
$X_1,\ldots,X_n \sim \Uniform(0,\theta)$. $X_{(n)} = \max X_i$ ile $\theta$'yı tahmin ediniz.
(a) $X_{(n)}$'nin PDF'sini bulunuz. (b) $E[X_{(n)}]$'i hesaplayınız.
...
(a) $\int_{-\infty}^{+\infty} |\varphi_X(t)|\,dt$'nin varlığını kontrol ediniz.
(b) Tersçevirme formülünü kullanarak $f_X(x)$'i hesaplayınız.
(c) Sonucun hangi standart dağılıma karşılık geldiğini belirtiniz.
\end{alistirma}
\alcozum{%
...
$g(x) = x^2 + 3x$.
(a) $g'(0)$ ve $g''(0)$'ı hesaplayınız.
(b) $\sqrt{n}(g(\bar{X}_n) - g(0))$ için limit dağılımı veriniz.
(c) Eğer $h(x) = x^2$ olsaydı, $n(h(\bar{X}_n) - h(0))$ için limit dağılımını veriniz.
\end{alistirma}
\alcozum{%
