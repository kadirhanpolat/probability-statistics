...
Yani büyük popülasyonda yerine koymadan örneklem $\approx$ yerine koyarak örneklem.
\end{onerme}
...
\kesifcevap{Üstel aile: $f(x;\theta)=h(x)\exp[\eta(\theta)T(x)-A(\theta)]$.
  Doğal parametre $\eta$, yeterli istatistik $T(x)$, log-bölümleme $A(\theta)$.
  Binom, Poisson, Normal, Gama, Beta hepsi bu kalıba girer.
  Üstel aile yapısı MLE analitiği, UMVUE (Rao-Blackwell) ve konjugat Bayes'i kolaylaştırır.}
\end{kesif}
...
\begin{ozyansima}
\begin{itemize}
  \item Binom ile Poisson arasındaki "limit" ilişkisini kendi cümlelerinizle açıklayınız.
...
% =====================================================================
\section{Weibull ve Pareto Dağılımları}
\label{sec:weibull_pareto}
% =====================================================================

\begin{motivasyon}[Üstel Ötesi: Esnek Ömür Modelleri]
Üstel dağılım "bellek yoksunluğu" ile idealdir --- ama gerçek cihaz ömürleri çoğunlukla
bunu sağlamaz: eskiyen sistemlerde arıza oranı zamanla \emph{artar}.
Weibull dağılımı bu esnekliği sağlar; şekil parametresi $k > 1$ azalan,
$k < 1$ artan, $k = 1$ sabit arıza oranını modeller.
\end{motivasyon}

\begin{tanim}[Weibull Dağılımı]
\label{def:weibull}
$X \sim \mathrm{Weibull}(k, \lambda)$ ($k, \lambda > 0$):
\[
  f(x) = \frac{k}{\lambda}\left(\frac{x}{\lambda}\right)^{k-1} e^{-(x/\lambda)^k}, \quad x > 0.
\]
\textbf{KDF:} $F(x) = 1 - e^{-(x/\lambda)^k}$.
\textbf{Beklenti:} $E[X] = \lambda\,\Gamma(1+1/k)$.
\textbf{Varyans:} $\mathrm{Var}(X) = \lambda^2\bigl[\Gamma(1+2/k) - \Gamma(1+1/k)^2\bigr]$.

Özel durum: $k = 1$ ise $X \sim \Exp(1/\lambda)$.
\end{tanim}

\begin{onerme}[Weibull --- Gama İlişkisi]
$X \sim \mathrm{Weibull}(k, \lambda)$ ise $Y = (X/\lambda)^k \sim \Exp(1)$.
Ters: $X = \lambda Y^{1/k}$. Bu dönüşüm simülasyonda kullanılır.
\end{onerme}

\begin{proof}
$P(Y \leq y) = P((X/\lambda)^k \leq y) = P(X \leq \lambda y^{1/k}) = 1 - e^{-y}$. \checkmark
\end{proof}

\begin{tanim}[Pareto Dağılımı]
\label{def:pareto}
$X \sim \mathrm{Pareto}(\alpha, x_m)$ ($\alpha, x_m > 0$):
\[
  f(x) = \frac{\alpha x_m^\alpha}{x^{\alpha+1}}, \quad x \geq x_m.
\]
\textbf{KDF:} $F(x) = 1 - (x_m/x)^\alpha$.
\textbf{Beklenti:} $E[X] = \alpha x_m/(\alpha-1)$ ($\alpha > 1$; yoksa $\infty$).
\textbf{Varyans:} $\mathrm{Var}(X) = \alpha x_m^2/[(\alpha-1)^2(\alpha-2)]$ ($\alpha > 2$).
\end{tanim}

\begin{ornek}[Pareto --- Güç Yasası]
$\alpha = 1.16$: Gelir dağılımı (80-20 kuralı --- Pareto ilkesi).
$P(X > x) = (x_m/x)^\alpha$: "ağır kuyruk" (heavy tail) --- olasılık polinomik azalır.
Üstel dağılımda $P(X > x) = e^{-\lambda x}$: üstel azalır.
Pareto'da $\alpha < 2$: sonsuz varyans; $\alpha < 1$: sonsuz beklenti.
\end{ornek}

\begin{hataanalizi}
"Pareto dağılımlı veriler için MLT'yi uygulayabilirim."
Yanlış: $\alpha \leq 2$ ise sonlu varyans yok; MLT'nin koşulu bozulur.
$\alpha \in (1,2)$: sonlu beklenti var ama sonlu varyans yok --- GBSK geçerli, MLT geçersiz.
$\alpha \in (0,1]$: sonlu beklenti bile yok --- GBSK bile çöker.
Ağır kuyruklu dağılımlarda istatistiksel yöntemler dikkatle seçilmelidir.
\end{hataanalizi}

\begin{disiplinlerarasibaglan}
\textbf{Güvenilirlik mühendisliği:} Weibull, ömür veri analizinde standart model;
şekil parametresi $k$ "arıza hızı profili"ni tam belirler.\\
\textbf{Finansal risk:} Pareto ve Fréchet dağılımları uç değer teorisinin
(Extreme Value Theory) temelini oluşturur; büyük kayıpların modellemesinde kullanılır.\\
\textbf{Sosyal bilimler:} Zipf yasası (kelime frekansı, şehir nüfusu) Pareto ile eşdeğerdir.
\end{disiplinlerarasibaglan}

% =====================================================================
\section{Karışım Dağılımları}
\label{sec:karisim_dagilim}
% =====================================================================

\begin{tanim}[Sonlu Karışım]
\label{def:karisim}
$f_1, \ldots, f_K$ PDF'ler, $\pi_k > 0$, $\sum_{k=1}^K \pi_k = 1$.
\textbf{Karışım dağılımı} (mixture distribution):
\[
  f(x) = \sum_{k=1}^K \pi_k f_k(x).
\]
$\pi_k$: karışım ağırlıkları. Beklenti: $E[X] = \sum_k \pi_k E_k[X]$.
Varyans: $\mathrm{Var}(X) = \sum_k \pi_k [\mathrm{Var}_k(X) + (E_k[X] - E[X])^2]$ (bileşke formülü).
\end{tanim}

\begin{proof}[Varyans Formülü]
$E[X^2] = \sum_k \pi_k E_k[X^2]$; $\mathrm{Var}(X) = E[X^2] - (E[X])^2$.
$\mathrm{Var}_k(X) = E_k[X^2] - (E_k[X])^2$, dolayısıyla $E_k[X^2] = \mathrm{Var}_k(X) + (E_k[X])^2$.
$\mathrm{Var}(X) = \sum_k\pi_k[\mathrm{Var}_k(X) + (E_k[X])^2] - (\sum_k\pi_k E_k[X])^2$;
son iki terimi toplama: $\sum_k\pi_k(E_k[X])^2 - (\sum_k\pi_k E_k[X])^2 = \sum_k\pi_k(E_k[X]-E[X])^2$.
\end{proof}

\begin{ornek}[Gaussian Karışımı]
$K=2$: $f(x) = \pi_1 \phi_{\mu_1,\sigma_1^2}(x) + \pi_2 \phi_{\mu_2,\sigma_2^2}(x)$.
Parametre: $(\pi_1, \mu_1, \sigma_1^2, \mu_2, \sigma_2^2)$.
İki tepe oluşturabilir --- tek Normal'in modelleyemediği bimodal veri.
Örnek: erkek-kadın boy dağılımı (iki normal, farklı ortalama).
\end{ornek}

\begin{ornek}[Negatif Binom --- Poisson Karışımı]
$X \mid \Lambda = \lambda \sim \Pois(\lambda)$, $\Lambda \sim \Gama(\alpha, \beta)$.
Marjinal: $P(X = k) = \int_0^\infty e^{-\lambda}\lambda^k/k! \cdot f_\Gamma(\lambda;\alpha,\beta)\,d\lambda$.
Hesap sonucu: $X \sim \mathrm{NegBinom}(\alpha, \beta/(1+\beta))$.
Bu "Negatif Binom = Poisson + Gama karışımı" yorumu, aşırı dağılımlı sayım verilerini açıklar.
\end{ornek}

\begin{teorem}[Karışım KDF'si]
$F(x) = \sum_k \pi_k F_k(x)$ (bileşen KDF'lerinin ağırlıklı ortalaması).
\end{teorem}

\begin{kritiksorgu}
Karışım dağılımı neden tek bileşenden daha esnek? Neden gerçek hayatta
karışım modelleri yaygın kullanılır? Karışımların MLE tahmini neden standart yöntemlerle
doğrudan yapılamaz? (İpucu: EM algoritması bu soruyu yanıtlar.)
\end{kritiksorgu}

% =====================================================================
\section{Çok Değişkenli Normal Dağılım}
\label{sec:cok_normal}
% =====================================================================

\begin{tanim}[Çok değişkenli normal]\label{def:cok_normal}
$\mathbf{X}=(X_1,\ldots,X_p)^\top$ \emph{çok değişkenli normal} dağılımlıdır, $\mathbf{X}\sim\Normal_p(\boldsymbol\mu,\boldsymbol\Sigma)$, her doğrusal kombinasyon $\mathbf{a}^\top\mathbf{X}$'in univariant normal dağılımlı olması koşuluyla.
\end{tanim}

\begin{teorem}[Çok değişkenli normal yoğunluğu]\label{thm:cok_normal_yogunluk}
$\boldsymbol\Sigma$ simetrik pozitif tanımlı ($p\times p$). $\mathbf{X}\sim\Normal_p(\boldsymbol\mu,\boldsymbol\Sigma)$:
\[
...
\begin{teorem}[Bağımsızlık koşulları]\label{thm:cok_normal_bagimsiz}
$\mathbf{X}\sim\Normal_p(\boldsymbol\mu,\boldsymbol\Sigma)$. Alt vektörler $\mathbf{X}_1,\mathbf{X}_2$ bağımsız $\iff$ $\Cov(\mathbf{X}_1,\mathbf{X}_2)=\mathbf{0}$ (korelasyonsuzluk = bağımsızlık, yalnızca normal için!).
\end{teorem}

\begin{proof}[Proof (taslak)]
MGF ayrışımı: $M_\mathbf{X}(\mathbf{t})=\exp(\boldsymbol\mu^\top\mathbf{t}+\tfrac12\mathbf{t}^\top\boldsymbol\Sigma\mathbf{t})$. $\Cov(\mathbf{X}_1,\mathbf{X}_2)=\mathbf{0}$ ise $\boldsymbol\Sigma$ blok köşegendir; MGF ayrışır: $M_{\mathbf{X}_1}(\mathbf{t}_1)\cdot M_{\mathbf{X}_2}(\mathbf{t}_2)$.
\end{proof}

\begin{ornek}[Koşullu normal]\label{ex:kosullu_normal}
$\mathbf{X}=(\mathbf{X}_1^\top,\mathbf{X}_2^\top)^\top\sim\Normal_{p+q}$, $\boldsymbol\Sigma=\begin{pmatrix}\boldsymbol\Sigma_{11}&\boldsymbol\Sigma_{12}\\\boldsymbol\Sigma_{21}&\boldsymbol\Sigma_{22}\end{pmatrix}$. Koşullu dağılım:
\[
  \mathbf{X}_1|\mathbf{X}_2=\mathbf{x}_2\sim\Normal_p\!\left(\boldsymbol\mu_1+\boldsymbol\Sigma_{12}\boldsymbol\Sigma_{22}^{-1}(\mathbf{x}_2-\boldsymbol\mu_2),\;\boldsymbol\Sigma_{11}-\boldsymbol\Sigma_{12}\boldsymbol\Sigma_{22}^{-1}\boldsymbol\Sigma_{21}\right).
\]
Koşullu ortalama, $\mathbf{x}_2$'nin doğrusal fonksiyonudur --- çok değişkenli regresyonun temeli.
\end{ornek}

\begin{teorem}[Kareler dağılımı: ki-kare bağlantısı]\label{thm:kare_dagilimlari}
$\mathbf{X}\sim\Normal_p(\mathbf{0},\mathbf{I}_p)$. O zaman $\|\mathbf{X}\|^2=\sum X_i^2\sim\chi^2_p$.
Genel: $(\mathbf{X}-\boldsymbol\mu)^\top\boldsymbol\Sigma^{-1}(\mathbf{X}-\boldsymbol\mu)\sim\chi^2_p$ ($\boldsymbol\Sigma$ pozitif tanımlı).
Bu, Mahalanobis mesafesinin dağılımıdır.
\end{teorem}

\begin{proof}
$\boldsymbol\Sigma=\mathbf{A}\mathbf{A}^\top$ (Cholesky). $\mathbf{Z}=\mathbf{A}^{-1}(\mathbf{X}-\boldsymbol\mu)\sim\Normal_p(\mathbf{0},\mathbf{I}_p)$. $\|\mathbf{Z}\|^2\sim\chi^2_p$.
\end{proof}

\begin{hataanalizi}
Genel olarak, korelasyonsuzluk bağımsızlık gerektirmez. Bu yalnızca normal dağılımda geçerlidir. $X\sim\Normal(0,1)$, $Y=X^2$: $\Cov(X,Y)=E[X^3]=0$ ama $Y$, $X$'e kesinlikle bağımlıdır.
\end{hataanalizi}

% =====================================================================
\section{Yüksek Boyutlu Dağılımlar: Wishart ve Hotelling}
\label{sec:wishart_hotelling}
% =====================================================================

\begin{tanim}[Wishart dağılımı]\label{def:wishart}
$\mathbf{X}_1,\ldots,\mathbf{X}_n\iid\Normal_p(\mathbf{0},\boldsymbol\Sigma)$. $\mathbf{W}=\sum_{i=1}^n\mathbf{X}_i\mathbf{X}_i^\top$ \emph{Wishart dağılımlıdır}: $\mathbf{W}\sim W_p(n,\boldsymbol\Sigma)$.
$\Beklenti[\mathbf{W}]=n\boldsymbol\Sigma$.
\end{tanim}

\begin{ornek}[Örneklem kovaryans matrisi]\label{ex:onek_kovaryans}
$\mathbf{X}_1,\ldots,\mathbf{X}_n\iid\Normal_p(\boldsymbol\mu,\boldsymbol\Sigma)$. $(n-1)\mathbf{S}=\sum_{i=1}^n(\mathbf{X}_i-\bar{\mathbf{X}})(\mathbf{X}_i-\bar{\mathbf{X}})^\top\sim W_p(n-1,\boldsymbol\Sigma)$.
$\mathbf{S}$ ve $\bar{\mathbf{X}}$ bağımsızdır (çok değişkenli Cochran teoremi).
\end{ornek}

\begin{tanim}[Hotelling $T^2$ istatistiği]\label{def:hotelling}
$T^2 = n(\bar{\mathbf{X}}-\boldsymbol\mu_0)^\top\mathbf{S}^{-1}(\bar{\mathbf{X}}-\boldsymbol\mu_0)$. $\boldsymbol\mu=\boldsymbol\mu_0$ altında: $\frac{n-p}{p(n-1)}T^2\sim F(p,n-p)$ (Hotelling dağılımı).
\end{tanim}

\begin{disiplinlerarasibaglan}
\textbf{Makine Öğrenmesi:} Çok değişkenli normal, Gaussian Naive Bayes ve LDA (Doğrusal Diskriminant Analizi) sınıflandırıcılarının temeli. Wishart, Bayesçi kovaryans tahminin konjugat önselidir.

\textbf{Nörobilim:} Mahalanobis mesafesi, beyin görüntüleme verilerinde sinyal gürültü ayırt etmede kullanılır; Hotelling $T^2$ çok değişkenli FMRI analizi testlerinin temelidir.
\end{disiplinlerarasibaglan}

\begin{mikroozet}
Weibull: esnek ömür modeli; $k=1$ üstel, $k>1$ artan arıza oranı.
Pareto: ağır kuyruk (heavy tail); $\alpha \leq 2$ sonsuz varyans.
Karışım: $f = \sum\pi_k f_k$; bileşke varyans = içsel + dışsal.
$\Normal_p$: korelasyonsuzluk $\iff$ bağımsızlık; koşullu dağılım $\Normal_p$; ki-kare bağlantısı.
Wishart: örneklem kovaryans matrisi dağılımı; Hotelling $T^2$: çok boyutlu $t$-testi.
\end{mikroozet}
...
(a) $P(X > 85)$. (b) $P(60 < X < 80)$. (c) 90. yüzdeliği bulunuz.
\end{alistirma}
\alcozum{%
...
$X \sim \Gama(3, 2)$.
(a) $E[X]$, $\mathrm{Var}(X)$ ve $M_X(t)$'yi yazınız.
(b) $P(X > 3)$'ü sayısal ifade ediniz (yarı analitik form yeterli).
(c) $X_1, X_2 \sim \Gama(3, 2)$ bağımsız ise $X_1 + X_2$'nin dağılımını bulunuz.
\end{alistirma}
\alcozum{%
...
$X \sim \Pois(\lambda)$'yı doğal parametre $\eta = \ln\lambda$ ile üstel aile formunda yazınız.
$B(\eta)$'yı bulunuz ve $B'(\eta) = E[T(X)]$ formülüyle $E[X] = \lambda$'yı doğrulayınız.
\end{alistirma}
\alcozum{%
...
\begin{alistirma}[Al.5.5cozum]{B}{Çok Değişkenli Normal: Koşullu Dağılım}
$(X,Y)\sim\Normal_2(\boldsymbol\mu,\Sigma)$; $\mu_X=2$, $\mu_Y=5$, $\sigma_X^2=4$, $\sigma_Y^2=9$, $\rho=0.6$.
(a) $Y|X=x$'in koşullu dağılımını yazınız.
(b) $E[Y|X=4]$ ve $\Var(Y|X=4)$'ü hesaplayınız.
(c) $X$ ve $Y-0.9X$ bağımsız mıdır? Neden?
\end{alistirma}
\alcozum{%
(a) Koşullu dağılım: $Y|X=x \sim \Normal(\mu_{Y|x},\,\sigma^2_{Y|x})$ ile
\[
  \mu_{Y|x} = \mu_Y + \rho\frac{\sigma_Y}{\sigma_X}(x-\mu_X) = 5 + 0.6\cdot\frac{3}{2}(x-2) = 5+0.9(x-2),
\]
\[
  \sigma^2_{Y|x} = \sigma_Y^2(1-\rho^2) = 9(1-0.36) = 5.76.
\]

(b) $x=4$: $\mu_{Y|4}=5+0.9(4-2)=5+1.8=6.8$; $\Var(Y|X=4)=5.76$. Dolayısıyla $Y|X=4\sim\Normal(6.8,5.76)$.

(c) $W=Y-0.9X$. $\Cov(X,W)=\Cov(X,Y)-0.9\Var(X)=\rho\sigma_X\sigma_Y-0.9\sigma_X^2=0.6\cdot2\cdot3-0.9\cdot4=3.6-3.6=0$. Ortak normal dağılımda kovaryans sıfır bağımsızlığa denk. Evet, $X$ ve $W=Y-0.9X$ bağımsız.}

\begin{alistirma}[Al.5.5b]{C}{Beta Dağılımı ve Bayes Güncellemesi}
$p\sim\Beta(\alpha,\beta)$ önsel. $n$ bağımsız Bernoulli($p$) denemesinde $x$ başarı gözlemleniyor.
(a) Sonsal $p|x$'i bulunuz.
(b) $\alpha=3$, $\beta=2$, $n=10$, $x=7$ için $E[p|x]$ ve $\mathrm{Mo}[p|x]$ (tepe) hesaplayınız.
(c) $\alpha+\beta\to\infty$ (sabit $\alpha/(\alpha+\beta)=p_0$) durumunda sonsal beklenti ne olur?
\end{alistirma}
\alcozum{%
(a) Olabilirlik: $L(p|x)=p^x(1-p)^{n-x}$. Önsel: $\pi(p)\propto p^{\alpha-1}(1-p)^{\beta-1}$. Sonsal:
\[
  \pi(p|x)\propto p^{x+\alpha-1}(1-p)^{n-x+\beta-1} \implies p|x\sim\Beta(\alpha+x,\,\beta+n-x).
\]
Konjugat aile özelliği.

(b) $\alpha'=3+7=10$, $\beta'=2+3=5$. $E[p|x]=\dfrac{10}{15}=\dfrac{2}{3}$; $\mathrm{Mo}[p|x]=\dfrac{\alpha'-1}{\alpha'+\beta'-2}=\dfrac{9}{13}\approx0.692$.

Karşılaştır: MLE $\hat p=7/10=0.700$. Bayes beklenti $2/3\approx0.667$ (önsel $p_0=3/5=0.6$'ya doğru çekilmiş).

(c) Önsel ağırlık $\alpha+\beta$, veri ağırlığı $n$. $E[p|x]=\dfrac{\alpha+x}{\alpha+\beta+n}\to\dfrac{\alpha/(\alpha+\beta)\cdot(\alpha+\beta)+x}{\alpha+\beta+n}\to p_0$ çünkü $\alpha+\beta\gg n$. Çok güçlü önsel; veri baskın gelemez.}
...
$X \sim \Beta(2, 3)$.
(a) $E[X]$ ve $\mathrm{Var}(X)$'i hesaplayınız.
(b) $P(X > 0.5)$'i analitik olarak bulunuz.
...
(a) $V = \sum X_i^2$'nin dağılımını yazınız; $E[V]$ ve $\mathrm{Var}(V)$'yi bulunuz.
(b) $Z \sim \Normal(0,1)$ bu $X_i$'lerden bağımsız. $T = Z/\sqrt{V/5}$'in dağılımını yazınız.
(c) $T^2$'nin dağılımını yazınız.
\end{alistirma}
\alcozum{%
...
$X$'in koşulsuz dağılımının negatif binom olduğunu gösteriniz.
\end{alistirma}
\alcozum{%
...
$Y \sim \mathrm{Cauchy}(\mu, \sigma)$ olduğunu gösteriniz.
...
$X \sim \Normal(\mu, \sigma^2)$'yi iki parametreli üstel ailede yazınız:
$\boldsymbol{\eta} = (\eta_1, \eta_2)$, $\mathbf{T}(x) = (T_1(x), T_2(x))$ ile.
$B(\boldsymbol{\eta})$'yı bulunuz ve $\partial B/\partial\eta_1 = E[T_1(X)]$,
$\partial B/\partial\eta_2 = E[T_2(X)]$ formüllerini doğrulayınız.
\end{alistirma}
\alcozum{%
...
Teorem~\ref{thm:binom_poisson}'in ispatında kullanılan iki limiti kesin biçimde ispatlayınız:
...
\begin{alistirma}[Al.5.13]{B}{Weibull Dağılımı}
$X \sim \mathrm{Weibull}(k, \lambda)$.
(a) $Y = (X/\lambda)^k$ dönüşümünün dağılımını bulunuz.
(b) $k = 2$, $\lambda = 1$ için $E[X]$ ve $\mathrm{Var}(X)$'i hesaplayınız
    ($\Gamma(1/2) = \sqrt{\pi}$, $\Gamma(3/2) = \sqrt{\pi}/2$, $\Gamma(2) = 1$).
(c) $P(X > 2)$'yi kapalı formda yazınız.
\end{alistirma}
\alcozum{%
(a) $P(Y \leq y) = P((X/\lambda)^k \leq y) = 1 - e^{-y}$: $Y \sim \Exp(1)$.\\
(b) $k=2$, $\lambda=1$: $E[X] = \Gamma(1+1/2) = \Gamma(3/2) = \sqrt{\pi}/2 \approx 0.886$.
$E[X^2] = \Gamma(1+1) = \Gamma(2) = 1$. $\mathrm{Var}(X) = 1 - \pi/4 \approx 0.215$.\\
(c) $P(X > 2) = e^{-(2/\lambda)^k} = e^{-4}$ ($k=2$, $\lambda=1$).}

\begin{alistirma}[Al.5.14]{B}{Pareto Beklentisi ve Varyansı}
$X \sim \mathrm{Pareto}(\alpha, x_m)$.
(a) $\alpha = 3$, $x_m = 1$ için $E[X]$ ve $\mathrm{Var}(X)$'i hesaplayınız.
(b) $P(X > 2x_m)$'yi $\alpha$ cinsinden yazınız.
(c) "80-20 kuralı": $\alpha = \log 5/\log 4 \approx 1.16$ ise nüfusun üst \%20'si
    toplam gelirin \%80'ini elde eder. $P(X > x_m \cdot 5^{1/\alpha})$'yi hesaplayınız.
\end{alistirma}
\alcozum{%
(a) $E[X] = \alpha x_m/(\alpha-1) = 3/2$ ($\alpha=3$, $x_m=1$).
$\mathrm{Var}(X) = \alpha x_m^2/[(\alpha-1)^2(\alpha-2)] = 3/(4\cdot1) = 3/4$.\\
(b) $P(X > 2x_m) = (x_m/(2x_m))^\alpha = (1/2)^\alpha = 2^{-\alpha}$.\\
(c) $P(X > x_m \cdot 5^{1/\alpha}) = (1/5^{1/\alpha})^\alpha = 1/5 = 0.20$.
Nüfusun \%20'si bu eşiğin üzerinde: 80-20 kuralı.}

\begin{alistirma}[Al.5.15]{A}{Gaussian Karışımı}
$f(x) = 0.3\,\phi(x+2) + 0.7\,\phi(x-1)$ ($\phi$ standart normal).
(a) $E[X]$'i hesaplayınız. (b) $\mathrm{Var}(X)$'i bileşke formülle hesaplayınız.
(c) PDF'nin tek modlu mu çift modlu mu olduğunu söyleyiniz.
\end{alistirma}
\alcozum{%
(a) $E[X] = 0.3 \cdot (-2) + 0.7 \cdot 1 = -0.6 + 0.7 = 0.1$.\\
(b) Bileşke: $\mathrm{Var}(X) = \sum_k\pi_k[\sigma_k^2 + (\mu_k - E[X])^2]$.
$= 0.3[1 + (-2-0.1)^2] + 0.7[1 + (1-0.1)^2]$
$= 0.3[1 + 4.41] + 0.7[1 + 0.81] = 0.3(5.41) + 0.7(1.81) = 1.623 + 1.267 = 2.890$.\\
(c) Modlar: $\mu_1 = -2$, $\mu_2 = 1$; fark $= 3 > 2\sigma = 2$. Çift modlu (bimodal).}
