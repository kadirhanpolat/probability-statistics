...
(a) $Y|X=x$'in koşullu dağılımını yazın.
(b) $E[Y|X=4]$ ve $\Var(Y|X=4)$'ü hesaplayın.
...
(a) Sonsal $p|x$'i bulun.
(b) $\alpha=3$, $\beta=2$, $n=10$, $x=7$ için $E[p|x]$ ve $\mathrm{Mo}[p|x]$ (tepe) hesaplayın.
...
(a) $Y = (X/\lambda)^k$ dönüşümünün dağılımını bulun.
(b) $k = 2$, $\lambda = 1$ için $E[X]$ ve $\mathrm{Var}(X)$'i hesaplayın
    ($\Gamma(1/2) = \sqrt{\pi}$, $\Gamma(3/2) = \sqrt{\pi}/2$, $\Gamma(2) = 1$).
(c) $P(X > 2)$'yi kapalı formda yazın.
\end{alistirma}
\alcozum{%
...
(a) $\alpha = 3$, $x_m = 1$ için $E[X]$ ve $\mathrm{Var}(X)$'i hesaplayın.
(b) $P(X > 2x_m)$'yi $\alpha$ cinsinden yazın.
(c) "80-20 kuralı": $\alpha = \log 5/\log 4 \approx 1.16$ ise nüfusun üst \%20'si
    toplam gelirin \%80'ini elde eder. $P(X > x_m \cdot 5^{1/\alpha})$'yi hesaplayın.
\end{alistirma}
\alcozum{%
...
(a) $E[X]$'i hesaplayın. (b) $\mathrm{Var}(X)$'i bileşke formülle hesaplayın.
(c) PDF'nin tek modlu mu çift modlu mu olduğunu söyleyin.
\end{alistirma}
\alcozum{%
