...
\textbf{P $\Rightarrow$ d:} $\varepsilon > 0$ ve $x$ süreklilik noktası alınız.
...
\kesifcevap{``$A_n$ sonsuz çok gerçekleşir'' $= \limsup_n A_n
  = \bigcap_{n=1}^\infty \bigcup_{k=n}^\infty A_k$.
  Sezgi: $\bigcup_{k \geq n} A_k$ ``$n$'inci adımdan sonra en az biri gerçekleşir''
  olayıdır; tüm $n$ için bu olayın gerçekleşmesi, sonsuz çok $A_k$'nın gerçekleşmesi demektir.}
\end{kesif}
...
\begin{itemize}
  ...
  \item Chernoff sınırının Chebyshev'den daha güçlü olduğunu bir örnekle gösteriniz.
\end{itemize}
\end{ozyansima}
...
Sabit $b > 1$. Geometrik alt dizi $n_k = \lfloor b^k \rfloor$ seçiniz.
...
\begin{kritiksorgu}
Ergodik teorem i.i.d.\ koşulunu gevşetiyor. Hangi koşulun gerektiğini tartışınız:
...
$X_n = n^{-1/2}Z$ ($Z \sim \Normal(0,1)$, sabit). Gösteriniz:
...
belirtiniz. (a) Chebyshev ZBSK ile $P(|\bar{X}_n - 1| > \varepsilon)$ için $n$ sayısına bağlı
üst sınır veriniz. (b) Bu yakınsamanın GBSK anlamında da geçerli olduğunu söyleyiniz.
\end{alistirma}
\alcozum{%
...
(a) Chebyshev ile $P(\bar{X}_{100} \geq 0.6)$ için üst sınır veriniz.
(b) Chernoff ile üst sınır veriniz ve karşılaştırınız.
\end{alistirma}
\alcozum{%
...
(b) Markov zinciri $X_n$ durağan dağılım $\pi$ ile ergodikse $\frac{1}{n}\sum_{k=1}^n f(X_k)\to\sum_x f(x)\pi(x)$ n.k. Bunu simülasyon bağlamında yorumlayınız.
...
(b) Markov Monte Carlo (MCMC) temeli: Zinciri durağan dağılımdan başlatınız (veya yeterince uzun ısın sonra) ve $\frac{1}{n}\sum f(X_k)$ ile $E_\pi[f]$'yi tahmin ediniz. Ergodiklik garantisi yakınsamayı sağlar; bağımsız örnekleme yerine geçer.
...
(a) $L(x)=\log x$ ve $L(x)=(\log x)^2$'nin yavaş değişen olduğunu doğrulayınız.
(b) $P(X>x)=L(x)/x^\alpha$ ($\alpha>0$) ise $X$'in ortalaması ne zaman sonlu?
(c) $\alpha=1$, $L(x)=1$ durumunda (Cauchy benzeri), $S_n/n\to?$ Limitin ne olduğunu belirtiniz.
\end{alistirma}
\alcozum{%
...
Her biri için örnek veriniz ya da imkânsız olduğunu gösteriniz:
...
$X_1, X_2, \ldots$ bağımsız. Aşağıdaki olayların olasılığını belirtiniz:
(a) $A = \{\bar{X}_n \to 0\}$.
...
Borel-Cantelli 1'i kullanarak $\bar{X}_{n^2} \xrightarrow{\text{n.k.}} \mu$ olduğunu ispatlayınız.
...
$T_n = X_n / S_n$'nin limit dağılımını bulunuz.
\end{alistirma}
\alcozum{%
...
Kronecker lemmasını kullanarak $\bar{X}_n \xrightarrow{\text{n.k.}} 0$ olduğunu gösteriniz.
(\textit{İpucu:} $S_n = \sum_{k=1}^n X_k/k$'nin n.k.\ yakınsadığını Borel-Cantelli ile kurunuz;
ardından Kronecker: $S_n \to S$ ve $S_n = \sum X_k/k$ ise $\frac{1}{n}\sum_{k=1}^n X_k \to 0$.)
\end{alistirma}
\alcozum{%
\begin{ipucu}{1}
$Y_k = X_k/k$ tanımlayınız; $\sum E[Y_k^2] = \sum \mathrm{Var}(X_k)/k^2 < \infty$.
...
(a) YLK'ya göre $\limsup_{n\to\infty} S_n/\sqrt{2n\ln\ln n}$'nin değerini veriniz.
(b) $n = 10^4$ için $\sqrt{2n\ln\ln n}$'yi sayısal olarak hesaplayınız.
...
(a) Temel Yenileme Teoremi ile $\lim_{t\to\infty} N(t)/t$'yi bulunuz.
(b) Blackwell teoremini kullanarak $[100, 101]$ aralığındaki beklenen yenileme sayısını yaklaşık hesaplayınız.
(c) $N(t)$'nin standart sapmasının $\sim \sqrt{\sigma^2 t/\mu^3}$ ölçeğinde olduğunu yorumlayınız
...
($0 < \alpha, \beta < 1$).
(a) Durağan dağılım $\pi$'yi bulunuz.
(b) $f(0) = 0$, $f(1) = 1$ ise Birkhoff Ergodik Teoremi'ne göre
    $\frac{1}{n}\sum_{k=0}^{n-1} f(Z_k)$'nın sınırını veriniz.
(c) Bu sonucu BSK ile karşılaştırınız: bağımsızlık gerekiyor mu?
\end{alistirma}
\alcozum{%
