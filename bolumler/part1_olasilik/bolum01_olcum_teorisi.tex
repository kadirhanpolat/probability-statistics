...
olayını $A_n$ ile gösteriniz. ``En az bir kez tura gelir'' olayı nedir?
...
\begin{proof}
\textbf{(i)} $A_{n+1} = A_{n+2} = \cdots = \emptyset$ koyup $\sigma$-toplamsallığı uygulayınız;
$\mu(\emptyset) = 0$ kullanınız.

\textbf{(ii)} $B = A \cup (B \setminus A)$ ayrık birleşim; (i)'den
$\mu(B) = \mu(A) + \mu(B \setminus A) \geq \mu(A)$.

\textbf{(iii)} $B_n = A_n \setminus \bigcup_{k=1}^{n-1} A_k$ tanımlayınız.
...
\textbf{(iv)} $B_1 = A_1$, $B_n = A_n \setminus A_{n-1}$ ($n \geq 2$) koyunuz.
...
\textbf{(v)} $C_n = A_1 \setminus A_n$ uygulayınız; $C_n \nearrow A_1 \setminus A$.
...
$\sim$ ile $x \sim y \iff x - y \in \mathbb{Q}$ denk bağıntısıyla sınıflandırınız.
Her denk sınıftan bir eleman seçerek $V$ oluşturunuz. $V$ ölçülebilir değildir:
...
tanımlayınız. $f_n$ ölçülebilir (her $A_k \in \mathcal{F}$), basit (sonlu değer alır),
...
\begin{proof}[İspat (özet)]
Tonelli teoremini $f^+$ ve $f^-$'ya ayrı ayrı uygulayınız.
...
$f(x,y) = e^{-x^2 - y^2}$ integralini hesaplayınız.
\[
...
\begin{ozyansima}
\begin{itemize}
  \item Bir $\sigma$-cebiri örnekleyiniz ve üç aksiyomu teker teker doğrulayınız.
  \item Neden $\mu(A_1) < \infty$ koşulu olmadan yukarıdan süreklilik çöker?
        Kendi örneğinizi üretiniz.
  \item MYT, Fatou ve DYT'yi karşılaştırınız: hangi durumlarda hangisini kullanırsınız?
  \item Fubini'nin neden $L^1$ koşulu gerektirdiğini karşı örnekle açıklayınız.
\end{itemize}
\end{ozyansima}
...
\text{ sayılabilirdir}\}$ ailesinin bir $\sigma$-cebiri olduğunu gösteriniz.
\end{alistirma}
\alcozum{%
...
$A_n = (0, 1/n)$ için $\lambda(A_n)$'i hesaplayınız ve
$\lambda\!\left(\bigcap_{n=1}^\infty A_n\right)$'yi yukarıdan süreklilik
teoremi ile bulunuz. Doğrudan hesapla doğrulayınız.
\end{alistirma}
\alcozum{%
...
$\int_0^\infty f_n \, d\lambda$'yı hesaplayınız ve MYT kullanarak
$\int_0^\infty e^{-x} \, d\lambda$'yı bulunuz.
\end{alistirma}
\alcozum{%
...
$n=2$ adımındaki $f_2$'yi açıkça yazınız ve $\|f - f_2\|_\infty$'u hesaplayınız.
\end{alistirma}
\alcozum{%
...
(a)~$\mathcal{F}_1 \cap \mathcal{F}_2$'nin $\sigma$-cebiri olduğunu gösteriniz.
(b)~$\mathcal{F}_1 \cup \mathcal{F}_2$'nin genel olarak $\sigma$-cebiri olmayabileceğini
    bir karşı örnekle gösteriniz.
\end{alistirma}
\alcozum{%
...
Gösteriniz ki $\mu(A \cup B) = \mu(A) + \mu(B) - \mu(A \cap B)$.
\end{alistirma}
\alcozum{%
...
$(\mathbb{R}, \mathcal{B}(\mathbb{R}))$ üzerinde ölçülebilir olduğunu gösteriniz.
\end{alistirma}
\alcozum{%
...
(a)~$\liminf_n f_n$'i bulunuz.
(b)~$\int \liminf_n f_n$ ile $\liminf_n \int f_n$'yi karşılaştırınız.
\end{alistirma}
\alcozum{%
...
(a)~$f_n(x) \to e^{-|x|}$ noktasal olarak gösteriniz.
(b)~Uygun dominant fonksiyonu bulunuz.
(c)~$\lim_n \int_{-\infty}^\infty f_n \, d\lambda$'yı hesaplayınız.
\end{alistirma}
\alcozum{%
...
önce $y$'ye göre integralleyerek, ardından $x$'e göre integralleyerek hesaplayınız.
...
yarı-halkalar üzerinde bir ön-ölçü (pre-measure) olduğunu gösteriniz ve
Carathéodory genişlemesiyle $\mathbb{R}$ üzerinde bir ölçü elde edildiğini açıklayınız.
...
$\mu(\{k\}) = 1/4$ (uniform), $\nu(\{1\}) = 1/2$, $\nu(\{2\}) = 1/4$, $\nu(\{3\}) = 1/8$, $\nu(\{4\}) = 1/8$.
(a) $\nu \ll \mu$ olduğunu doğrulayınız. (b) Radon-Nikodym türevi $f = d\nu/d\mu$'yu bulunuz.
(c) $\nu(A) = \int_A f \, d\mu$ olduğunu $A = \{1,3\}$ için doğrulayınız.
\end{alistirma}
\alcozum{%
...
(a) $\|f\|_p = (\int_0^\infty e^{-px} dx)^{1/p}$'yi hesaplayınız.
(b) $p \to \infty$ limitinde $\|f\|_p$'nin ne olduğunu yorumlayınız.
(c) Hölder eşitsizliğiyle $\int_0^\infty e^{-x}\cos x \, dx \leq \|f\|_2\|\cos\|_{L^2([0,\infty))}$
    üst sınırını veriniz (dikkat: $\|\cos\|_{L^2([0,\infty))} = \infty$; bu ne anlama gelir?).
\end{alistirma}
\alcozum{%
